...
if(@$GLOBALS['status']==2 || @$GLOBALS['wlasciciel']==@$GLOBALS['nazwaus'] || @$_GET['id']=='' || !isset($_GET['id'])){
	...
	if(is_uploaded_file($plik_tmp)){
		if ($plik_rozmiar > 30000000){
			...
			$i=0;
			do{
				$plik = $kid.'/'.htmlspecialchars($plik_nazwa);
				...
				if(!$test){
					...
					while($y<$ostatnia_czesc){
						$plik_nazwa=htmlspecialchars($plik_nazwa.$rozszerzenie[$y]);
			...
			move_uploaded_file($plik_tmp, htmlspecialchars($kid).'/'.htmlspecialchars($plik_nazwa));
			if($roz == 'php'){
	...
	public function tworzRozszerzenie(){	
   	...
   	public function tworzGeshi(){	
		if ($this->rozszerzeniePliku == 'c++' or $this->rozszerzeniePliku == 'cpp' or $this->rozszerzeniePliku == 'c' or 
			...
			$this->rozszerzeniePliku == 'css' or $this->rozszerzeniePliku == 'sql' or $this->rozszerzeniePliku == 'xml' or $this->rozszerzeniePliku == 'py')
			{$this->geshiPliku=1;}
		else 
			{$this->geshiPliku=0;}
	}
   	public function tworzHtml($folder){	
		if ($this->rozszerzeniePliku=='py') $rozxx='python';
		else $rozxx = $this->rozszerzeniePliku;
		if($this->geshiPliku==1){
			$this->htmlPliku = '<div class="ikona"><a href="strona/geshi.php?url=../'.$folder.'/'.
								$this->nazwaPliku.'&amp;lang='.$rozxx.'"><img src="img/ico/';
		}
		else{
		...
		elseif ($this->rozszerzeniePliku == 'py') {$this->htmlPliku .= 'python.png';}
		...
		elseif ($this->rozszerzeniePliku == 'py' or $this->rozszerzeniePliku == 'cpp')   {$this->htmlPliku .= 'cpp.png';}
		...
		if($this->geshiPliku==1){
			$this->htmlPliku .= '" alt="" title="'.$this->nazwaPlikuCzysta.'"/></a><br><strong><a href="strona/geshi.php?url=../'.$folder.'/'.$this->nazwaPliku.
								'&amp;lang='.$rozxx.'">'.$this->nazwaPlikuSkrocona.'</a></strong><br>';
...
<body>
	...
	<nav>
		...
		<section><!--///////////MENU-->
			...
			<a href="html5.php"><img src="img/st.png" alt=""> html5</a><br>
	...
	<div id="contents">
		<article>
			...
			<?php 
				$tmp = mysql_fetch_array(mysql_query('select * from folders where adres="'.@$_GET['id'].'"'));
				if(@$tmp[haslo]!='' && @$_COOKIE[$tmp[adres]]!=@$tmp[haslo] && @$_POST['haslof']!=@$tmp[haslo]){
					echo '&nbsp;<br><form method="post" action="index.php?id='.@$_GET['id'].'">
					<strong>Podaj hasło: </strong>
					<input type="text" name="haslof" value="" class="formreg"/>
					<input type="submit" name="upload" id="formlogokup" value="OK"/>
					</form><br>&nbsp;<div class="clear"></div>';
				}
				else{
					$tmp = mysql_fetch_array(mysql_query('select * from folders where adres="'.@$_GET['id'].'"'));
					if($tmp['text']!=''){
						echo'<a style="float:center">'.$tmp['text'].'</a><br>';
					}
					echo foldery().pliki();
				}
			?>
			...
			<div class="bgdown"></div>
			<!--/////////////BLOK NIZEJ////////////////-->
			<?php if(isset($_GET['nnn'])) {
				if(isset($_GET['id']) && $_GET['id']!=''){$tt='id='.@$_GET['id'].'&';}
				else{$tt='';}
			echo'<section class="uploadok">
				<form method="post" action="index.php?'.$tt.'op=nn&nnnazwa='.@$_GET['nnnazwa'].'">
					<strong>Przenieś do folderu: </strong>
					<input type="text" name="nnnazwa" value="" class="formreg"/>
					<input type="submit" name="upload" id="formlogokup" value="Przenieś"/>
				</form><div class="clear"></div>
			</section>
			<div class="bgdown"></div>';}?>
			<!--/////////////BLOK ZMIEN NAZWE////////////////-->
			<?php if(isset($_GET['znn'])) {
			echo'<section class="uploadok">
				<form method="post" action="index.php?id='.@$_GET['id'].'&op=zn&nnazwa='.@$_GET['nnazwa'].'">
					<strong>Zmień nazwę: </strong>
					<input type="text" name="ninazwa" value="'.$_GET['nnazwa'].'" class="formreg"/>
					<input type="submit" name="upload" id="formlogokup" value="Zmień"/>
				</form><div class="clear"></div>
			</section>
			<div class="bgdown"></div>';}?>
				<!--/////////////BLOK ZARZADZANIA KATALOGIEM////////////////-->
			<?php 
			if($GLOBALS['status']==2 || $GLOBALS['wlasciciel']==$GLOBALS['nazwaus']){
			if(isset($_GET['id'])){
			$tmp = mysql_fetch_array(mysql_query('select * from folders where adres="'.$_GET['id'].'"'));
			echo'<div class="bgup"></div>
			<section class="uploadok">
				<form method="post" action="index.php?op=df&id='.$_GET['id'].'">
					<strong>Zarządzanie katalogiem: </strong>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;Hasło: <input type="text" name="fopas" value="'.$tmp['haslo'].'" class="formreg"/>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Dostęp: <input type="text" name="fodos" value="'.$tmp['dostep'].'" class="formreg"/>
					<input type="submit" name="upload" id="formlogokupc" value="Akceptuj""/><br>
					Info: <input type="text" name="foinf" value="'.$tmp['text'].'" id="formregj";/>
				</form><div class="clear"></div>
			</section>
			<div class="bgdown"></div>';}}?>
					<!--/////////////////NOWY KATALOG-->
			...
			<section class="uploadok">
				...
				if(isset($_GET['id'])){$tt='id='.@$_GET['id'].'&';}else{$tt='';}echo 'index.php?'.$tt.'op=cf';
				echo'">
					<strong>Utwórz katalog: </strong>
					<input type="text" name="foldeer" value="" class="formreg"/>
					<input type="submit" name="upload" id="formlogokup" value="Utwórz"/>
				</form><div class="clear"></div>
			</section>
			<div class="bgdown"></div>';}?>
					<!--/////////////////UPLOAD-->
					...
					if(@$GLOBALS['status']==2 || @$GLOBALS['wlasciciel']==@$GLOBALS['nazwaus'] || @$_GET['id']=='' || !isset($_GET['id'])){
					if(!czyUser()){
						if(!isset($_GET['id']) || $_GET['id']=='') {$kk='';}
						else{$kk='id='.$_GET['id'];}
						echo '<div class="bgup"></div><section class="uploadok">';
						$tr1 = '<form enctype="multipart/form-data" action="index.php?'.$kk.'" method="POST"><strong>';//upload plików
						$tr2 = '</strong><input type="hidden" name="MAX_FILE_SIZE" value="10000000000"/>
									<input type="file" name="plik" class="upload" />
									<input type="submit" name="upload" id="formlogokup" value="Wrzuć""/>
								</form>';
						if (@$GLOBALS['op']==3){
							echo $tr1.'Nie wybrałeś pliku'.$tr2;
						}
						elseif (@$GLOBALS['op']==2){
							echo 'Plik: <strong>'.$plik_nazwa.'</strong> o rozmiarze <strong>'.number_format($plik_rozmiar/(1024*1024), 2, '.', '').' MB</strong> został przesłany na serwer!<br>
									Link do pliku to: <br><strong><a href="http://acyk.pl/'.$kid.'/'.$plik_nazwa.'">http://acyk.pl/'.$kid.'/'.$plik_nazwa.'</a></strong>';
							echo '</section>
									<div class="bgdown"></div><div class="bgup"></div>
									<section class="uploadok">'.$tr1.'Upload plików'.$tr2.'<div class="clear"></div>
									</section>
									<div class="bgdown"></div>';
						}
						elseif(@$GLOBALS['op']==1){
							echo $tr1.'Plik jest zaduży'.$tr2;
						}
						else{
							echo $tr1.'Upload plików'.$tr2;
						}
						...
						echo'<div class="bgup"></div>
									<section class="uploadok">Aby dodawać pliki trzeba być zalogowanym.<div class="clear"></div>
									</section>
									<div class="bgdown"></div>';
					}
					}
	...
	<footer id="finish"><!--stopka-->
		<p>Copyright &copy; 2013. Created by Paweł Czubak.<br>Valid <a href="">HTML 5</a> and <a href="">CSS 3</a>.</p>
	</footer>
</body>
</html>